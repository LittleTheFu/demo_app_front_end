...
export const CommentGroup: React.FC<CommentGroupProps> = (props: CommentGroupProps) => {
    const { comments, pages, pageNum,
        authorClick, thumbClick, unThumbClick, onPageChange } = props;
        ...
        {comments.map((comment: ArticleComment, index: number) => {
            return (
                <CommentCard key={index}
                    ...
                    thumbClick={() => { thumbClick(comment.id) }}
                    unThumbClick={() => { unThumbClick(comment.id) }} />
            );
        })}
    </div>)
